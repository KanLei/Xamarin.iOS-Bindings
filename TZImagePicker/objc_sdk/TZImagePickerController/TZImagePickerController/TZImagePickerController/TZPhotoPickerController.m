...
- (void)viewDidLoad {
    ...
    //mmtime 自定义取消按钮  带添加图片
      self.navigationItem.rightBarButtonItem.tintColor = [UIColor blackColor];
    
    _showTakePhotoBtn = (([[TZImageManager manager] isCameraRollAlbum:_model.name]) && tzImagePickerVc.allowTakePicture);
    _showTakeMovieBtn= (([[TZImageManager manager] isCameraRollAlbum:_model.name]) && tzImagePickerVc.allowPickingVideo);
    if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
        ...
        if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear||_showTakeMovieBtn) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.row >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.row == 0)) && _showTakeMovieBtn) {
        TZAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TZAssetCameraCell" forIndexPath:indexPath];
        cell.imageView.image = [UIImage imageNamedFromMyBundle:tzImagePickerVc.takeVideoImageName];  // mmtime: 使用拍摄视频图片
        return cell;
    }
    ...
    TZAssetModel *model;
        if(_showTakeMovieBtn&&!tzImagePickerVc.sortAscendingByModificationDate){
            model = _models[indexPath.row-1];
        }else if(_showTakePhotoBtn&&!tzImagePickerVc.sortAscendingByModificationDate){
            model = _models[indexPath.row-1];
        }else {
...
- (void)callDelegateMethod {
    ...
    if ([imagePickerVc.pickerDelegate respondsToSelector:@selector(takeMoviePlay:oldViewController:)]) {
        [imagePickerVc.pickerDelegate takeMoviePlay:imagePickerVc oldViewController:self];
    }
    //if (imagePickerVc.didFinishPickingVideoHandle) {
      //  imagePickerVc.pickerDelegate.takeMoviePlay();//mmtime 视频选择完成回调函数；
    //}
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!tzImagePickerVc.sortAscendingByModificationDate && _showTakeMovieBtn) {
        index = indexPath.row - 1;
    }
...
- (void)scrollCollectionViewToBottom {
    ...
    if (_shouldScrollToBottom && _models.count > 0 && tzImagePickerVc.sortAscendingByModificationDate) {
        ...
        if (_showTakePhotoBtn||_showTakeMovieBtn) {
            TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
