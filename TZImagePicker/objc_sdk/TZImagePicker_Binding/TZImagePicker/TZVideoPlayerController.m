...
- (void)doneButtonClick {
    ...
    if (self.navigationController) {
        ...
        {
            [self callDelegateMethod];  // mmtime: 增加禁用 AutoDismiss 回调
        }
    } else {
...
- (void)pausePlayerAndShowNaviBar {
    [_player pause];
    _toolBar.hidden = NO;
    //    [self.navigationController setNavigationBarHidden:NO];  // mmtime: 隐藏 navbar
    [_playButton setImage:[UIImage imageNamedFromMyBundle:@"MMVideoPreviewPlay.png"] forState:UIControlStateNormal];
//    if (iOS7Later) [UIApplication sharedApplication].statusBarHidden = NO;  // mmtime: 隐藏 status bar
}
